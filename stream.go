...
var emptyHdr = [5]byte{0, 0, 0, 0, 0}

func (cs *clientStream) SendRaw(m []byte) (err error) {
	defer func() {
		if err != nil && err != io.EOF {
			// Call finish on the client stream for errors generated by this SendMsg
			// call, as these indicate problems created by this client.  (Transport
	...
	op := func(a *csAttempt) error {
		return a.sendMsg(m, hdr, payload, nil)
	}
	err = cs.withRetry(op, func() { cs.bufferForRetryLocked(len(hdr)+len(payload), op) })
	if len(cs.binlogs) != 0 && err == nil {
		cm := &binarylog.ClientMessage{
...
func (cs *clientStream) RecvRaw() ([]byte, error) {
	if len(cs.binlogs) != 0 && !cs.serverHeaderBinlogged {
		// Call Header() to binary log header if it's not already logged.
		cs.Header()
	}
...
func (a *csAttempt) recvRaw(payInfo *payloadInfo) (buf []byte, err error) {
	cs := a.cs
	if len(a.statsHandlers) != 0 && payInfo == nil {
		payInfo = &payloadInfo{}
	}
	...
	buf, err = recvRaw(a.p, a.s, *cs.callInfo.maxReceiveMessageSize, payInfo)
	if err != nil {
		if err == io.EOF {
			if statusErr := a.s.Status().Err(); statusErr != nil {
	...
	if a.trInfo != nil {
		a.mu.Lock()
		if a.trInfo.tr != nil {
			a.trInfo.tr.LazyLog(&payload{sent: false, msg: buf}, true)
		}
		a.mu.Unlock()
	}
	for _, sh := range a.statsHandlers {
	...
	// Special handling for non-server-stream rpcs.
	// This recv expects EOF or errors, so we don't collect inPayload.
	buf, err = recvRaw(a.p, a.s, *cs.callInfo.maxReceiveMessageSize, nil)
	if err == nil {
		return nil, toRPCErr(errors.New("grpc: client streaming protocol violation: get <nil>, want <EOF>"))
	}
	if err == io.EOF {
		return buf, a.s.Status().Err() // non-server streaming Recv returns nil on success
	}
	return nil, toRPCErr(err)
}
...
func (as *addrConnStream) SendRaw(m []byte) (err error) {
	defer func() {
		if err != nil && err != io.EOF {
			// Call finish on the client stream for errors generated by this SendMsg
			// call, as these indicate problems created by this client.  (Transport
...
func (as *addrConnStream) RecvRaw() (buf []byte, err error) {
	defer func() {
		if err != nil || !as.desc.ServerStreams {
			// err != nil or non-server-streaming indicates end of stream.
			as.finish(err)
	...
	buf, err = recvRaw(as.p, as.s, *as.callInfo.maxReceiveMessageSize, nil)
	if err != nil {
		if err == io.EOF {
			if statusErr := as.s.Status().Err(); statusErr != nil {
				return nil, statusErr
			}
			return buf, io.EOF // indicates successful end of stream.
		}
		return nil, toRPCErr(err)
	}
	...
	if err == io.EOF {
		return buf, as.s.Status().Err() // non-server streaming Recv returns nil on success
	}
	return nil, toRPCErr(err)
}
...
func (ss *serverStream) SendRaw(m []byte) (err error) {
	defer func() {
		if ss.trInfo != nil {
			ss.mu.Lock()
			if ss.trInfo.tr != nil {
...
func (ss *serverStream) RecvRaw() (buf []byte, err error) {
	defer func() {
		if ss.trInfo != nil {
			ss.mu.Lock()
			if ss.trInfo.tr != nil {
				if err == nil {
					ss.trInfo.tr.LazyLog(&payload{sent: false, msg: buf}, true)
				} else if err != io.EOF {
					ss.trInfo.tr.LazyLog(&fmtStringer{"%v", []any{err}}, true)
					ss.trInfo.tr.SetError()
				}
	...
	if buf, err = recvRaw(ss.p, ss.s, ss.maxReceiveMessageSize, payInfo); err != nil {
		if err == io.EOF {
			if len(ss.binlogs) != 0 {
				chc := &binarylog.ClientHalfClose{}
				for _, binlog := range ss.binlogs {
